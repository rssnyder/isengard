...
function addbot () {
  ...
  curl -X PUT -s "192.168.254.24:7777/bot/free?bot_id=$1"  | jq -e '.id' || curl -X POST -s "192.168.254.24:7777/bot/store?bot_id=$1&bot_token=$2" | jq -e '.id'
}

function newbots () {
  curl -setting "192.168.254.24:7777/bot/unclaimed" | jq .
}

function popnewbot () {
  newbot=$(curl -s '192.168.254.24:7777/bot/get' | jq -c .)
  id=$(echo $newbot | jq -r .id)
  token=$(echo $newbot | jq -r .token)
  echo $newbot | jq .
  curl -X PUT -s "192.168.254.24:7777/bot/claim?bot_id=$id" | jq -c .
}
...
function newcrypto () {
  ...
  curl -X PUT -s "192.168.254.24:7777/bot/claim?bot_id=$id" | jq -c .
  curl -X PUT -s "192.168.254.24:7777/bot/photo?bot_id=$id&photo_url=$(curl -s https://api.coingecko.com/api/v3/coins/$2 | jq -r .image.large | cut -d'?' -f1)"
}
...
function botimage () {
  ...
  curl -X PUT "192.168.254.24:7777/bot/photo?bot_id=$1&photo_url=$2"
}
